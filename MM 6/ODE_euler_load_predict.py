...
if __name__ == "__main__":
    ...
    ttest = np.asarray(pd.read_csv('./data/ttest.csv'))[:, :, np.newaxis]
    xttest = np.asarray(pd.read_csv('./data/xttest.csv'))

    model = create_model(x0=convert_to_tensor(x0, dtype=float32), fLayer=fLayer,
                         batch_input_shape=t_train.shape)
    xtBefore = model.predict_on_batch(ttest)[:, 0]

    # loading weights from trained model
    model.load_weights("./savedmodels/cp.ckpt")
    xtAfter = model.predict_on_batch(ttest)[:, 0]

    # plotting predictions
    fig = plt.figure()
    plt.plot([0, 4], [0, 4], '--k')
    plt.plot(xttest[:], xtBefore[:], 'o', label='before training')
    plt.plot(xttest[:], xtAfter[:], 's', label='after training')
